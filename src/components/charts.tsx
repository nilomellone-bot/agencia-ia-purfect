...
export function SalesChart({days=14,rows}:{days?:number;rows?:{day:string;sales:number;spend:number}[]}){
 const id=useId().replaceAll(':','');
 return <div className="sales-chart" role="img" aria-label={`Gráfico de ventas y publicidad ${rows?"cargadas":"de ejemplo"} en pesos argentinos, con dos escalas independientes`}><ResponsiveContainer width="100%" height="100%" minWidth={0}>
  <AreaChart data={rows??dailyData.slice(-days)} margin={{top:12,right:5,left:0,bottom:0}}>
